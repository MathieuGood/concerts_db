...
async function onArtistSave(event: any) {
  ...
  if (idx !== -1) artistRows.value[idx] = { ...artistRows.value[idx]!, name: newData.name, country_id: newData.country_id, country, countryName: country?.name ?? '' }
}
...
<template>
  <div>
    ...
    <template v-else>
      ...
      <DataTable :value="filtered" dataKey="id" sortField="concerts" :sortOrder="-1"
        v-model:expandedRows="expandedRows" editMode="row" v-model:editingRows="editingRows" @row-edit-save="onArtistSave"
        ...
        <Column style="width:3rem">
          <template #body="{ data }">
            <Button v-if="editingRows.find(r => r.id === data.id)" icon="pi pi-trash" text rounded severity="danger" size="small" @click="onArtistDelete(data)" />
          </template>
        </Column>
