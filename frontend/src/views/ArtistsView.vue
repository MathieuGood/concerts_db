...
function searchNewArtistCountry(event: { query: string }) {
  ...
  newArtistCountrySuggestions.value = countries.value.filter(c => normalize(c.name).includes(q))
}
  ...
  <div class="space-y-4">
    ...
    <template v-else>
      <div class="flex gap-2">
        <IconField class="flex-1"><InputIcon class="pi pi-search" /><InputText v-model="search" placeholder="Search artists…" class="w-full" /><InputIcon v-if="search" class="pi pi-times cursor-pointer" style="pointer-events:auto" @click="search = ''" /></IconField>
      ...
      <div v-if="addingArtist" class="flex gap-2 p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
