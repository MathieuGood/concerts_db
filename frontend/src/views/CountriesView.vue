...
onMounted(async () => {
  try {
    ...
    countryRows.value = countries.map(c => {
      ...
      return { id: c.id, name: c.name, events: s?.events ?? 0, cities: s?.cityIds.size ?? 0, venues: s?.venueIds.size ?? 0, artists: s?.artistIds.size ?? 0, firstVisit: s?.first ?? null, lastVisit: s?.last ?? null, eventList: s?.eventList.sort((a, b) => a.date.localeCompare(b.date)) ?? [], _editing: false }
    })
    expandedRows.value = countryRows.value.filter(r => initialExpandedIds.includes(r.id))
    expandedCards.value = initialExpandedIds.filter(id => countryRows.value.some(r => r.id === id))
...
async function onSave(event: any) {
  ...
  if (idx !== -1) countryRows.value[idx] = { ...countryRows.value[idx]!, name: String(newData.name), _editing: false }
}
  ...
  <div class="space-y-4">
    ...
    <template v-else>
      <div class="flex gap-2">
        <IconField class="flex-1"><InputIcon class="pi pi-search" /><InputText v-model="search" placeholder="Search countries…" class="w-full" /><InputIcon v-if="search" class="pi pi-times cursor-pointer" style="pointer-events:auto" @click="search = ''" /></IconField>
              ...
              <div class="flex-1 min-w-0 cursor-pointer" @click="toggleExpand(row.id)">
                ...
                <div class="flex flex-wrap gap-1.5 mt-2">
                  <span class="inline-flex items-center gap-1 bg-gray-100 dark:bg-gray-800 px-2 py-0.5 rounded-full text-xs">
                    <span class="font-semibold text-d-orange">{{ row.events }}</span><span class="text-gray-500">shows</span>
                  </span>
                  <span class="inline-flex items-center gap-1 bg-gray-100 dark:bg-gray-800 px-2 py-0.5 rounded-full text-xs">
                    <span class="font-semibold text-d-orange">{{ row.cities }}</span><span class="text-gray-500">cities</span>
                  </span>
                  <span class="inline-flex items-center gap-1 bg-gray-100 dark:bg-gray-800 px-2 py-0.5 rounded-full text-xs">
                    <span class="font-semibold text-d-orange">{{ row.venues }}</span><span class="text-gray-500">venues</span>
                  </span>
                </div>
            ...
            <div class="px-4 py-4">
              <div class="flex flex-wrap gap-2 mb-4">
                <span class="inline-flex items-center gap-1.5 bg-gray-100 dark:bg-gray-800 px-3 py-1 rounded-full text-sm">
                  <span class="font-semibold text-d-orange">{{ data.events }}</span><span class="text-gray-500">events</span>
                </span>
                <span class="inline-flex items-center gap-1.5 bg-gray-100 dark:bg-gray-800 px-3 py-1 rounded-full text-sm">
                  <span class="font-semibold text-d-orange">{{ data.cities }}</span><span class="text-gray-500">cities</span>
                </span>
                <span class="inline-flex items-center gap-1.5 bg-gray-100 dark:bg-gray-800 px-3 py-1 rounded-full text-sm">
                  <span class="font-semibold text-d-orange">{{ data.venues }}</span><span class="text-gray-500">venues</span>
                </span>
                <span class="inline-flex items-center gap-1.5 bg-gray-100 dark:bg-gray-800 px-3 py-1 rounded-full text-sm">
                  <span class="font-semibold text-d-orange">{{ data.artists }}</span><span class="text-gray-500">artists</span>
                </span>
              </div>
