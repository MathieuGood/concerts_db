  ...
  <div>
    ...
    <template v-else>
      ...
      <DataTable :value="filtered" dataKey="id" sortField="events" :sortOrder="-1"
        ...
        <Column field="cities" header="Cities" sortable style="width:80px">
          <template #body="{ data }"><span :class="data.cities > 0 ? '' : 'text-gray-400'">{{ data.cities || '—' }}</span></template>
        </Column>
        <Column field="venues" header="Venues" sortable style="width:80px">
          <template #body="{ data }"><span :class="data.venues > 0 ? '' : 'text-gray-400'">{{ data.venues || '—' }}</span></template>
        </Column>
        <Column field="artists" header="Artists" sortable style="width:80px">
