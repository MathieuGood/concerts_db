...
<template>
  <header class="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 sticky top-0 z-50">
    <div class="max-w-4xl mx-auto px-4 h-14 flex items-center justify-between">
      <button class="text-xl text-violet-600 dark:text-violet-400 cursor-pointer"
        style="font-family: 'DepartureMono', monospace;" @click="navigate('/')">
        ConcertsDB
      </button>
      ...
      <div class="hidden sm:flex items-center gap-2">
        ...
        <Button v-for="link in libraryLinks" :key="link.path" :icon="`pi ${link.icon}`" size="small" rounded
          :text="route.path !== link.path" :severity="route.path === link.path ? undefined : 'secondary'"
          @click="navigate(link.path)" :aria-label="link.label" />

        <Button v-if="isAdmin" icon="pi pi-database" size="small" rounded :text="route.path !== '/import'"
          :severity="route.path === '/import' ? undefined : 'secondary'" @click="navigate('/import')"
          aria-label="Import / Export" />
        <Button v-if="isAdmin" icon="pi pi-shield" size="small" rounded :text="route.path !== '/admin'"
          :severity="route.path === '/admin' ? undefined : 'secondary'" @click="navigate('/admin')"
      ...
      <div class="flex sm:hidden items-center gap-1">
        <Button :icon="isDark ? 'pi pi-sun' : 'pi pi-moon'" size="small" rounded text @click="toggleTheme"
          :aria-label="isDark ? 'Switch to light mode' : 'Switch to dark mode'" />
        <Button :icon="menuOpen ? 'pi pi-times' : 'pi pi-bars'" size="small" rounded text severity="secondary"
    ...
    <div v-if="menuOpen"
      ...
      <button
        ...
        :class="route.path === '/'
          ? 'badge-d-purple'
          : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800'" @click="navigate('/')">
        <i class="pi pi-calendar" />
        Shows
      </button>

      <button v-for="link in libraryLinks" :key="link.path"
        class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium w-full text-left transition-colors"
        :class="route.path === link.path
          ? `badge-d-${link.color}`
          : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800'" @click="navigate(link.path)">
      ...
      <button v-if="isAdmin"
        ...
        :class="route.path === '/import'
          ? 'badge-d-comment'
          : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800'" @click="navigate('/import')">
      ...
      <button v-if="isAdmin"
        ...
        :class="route.path === '/admin'
          ? 'badge-d-comment'
          : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800'" @click="navigate('/admin')">
